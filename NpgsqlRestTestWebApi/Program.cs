...
int? GetInt(string key, IConfigurationSection? subsection = null)
{
    var section = subsection?.GetSection(key) ?? config?.GetSection(key);
...
T? GetEnum<T>(string key, IConfigurationSection? subsection = null)
{
    var section = subsection?.GetSection(key) ?? config?.GetSection(key);
...
static string CreateUrl(Routine routine, NpgsqlRestOptions options) => string.Concat(
    string.IsNullOrEmpty(options.UrlPathPrefix) ? "/" :
        string.Concat("/", options.UrlPathPrefix.Trim('/'), "/"),
    routine.Schema == "public" ? "" : routine.Schema.Trim('"'),
    "/",
    routine.Name.Trim('"'),
    "/");
