...
In previous versions of the library, only PostgreSQL functions and procedures were considered as routine sources. The REST API was built on the available functions and procedures in the PostgreSQL database and provided configuration.
...
{
    ConnectionString = connectionString,
    EndpointCreateHandlers = [new HttpFile(), new TsClient()],
});
app.Run();
```
